...
export default function Page({  }) {
  ...
  const turtle_market = calculateMarketValues(0, 0, 0);
  const froggie_market = calculateMarketValues(0, 0, 0);
  const konda_market = calculateMarketValues(0, 0, 0);
  const catsky_market = calculateMarketValues(0, 0, 0);
  const agix_market = calculateMarketValues(0, 0, 0);
  const snek_market = calculateMarketValues(0, 0, 0);
  const wmt_market = calculateMarketValues(0, 0, 0);
  const meld_market = calculateMarketValues(0, 0, 0);
  const min_market = calculateMarketValues(0, 0, 0);
  const aada_market = calculateMarketValues(0, 0, 0);
  const ntx_market = calculateMarketValues(0, 0, 0);
  const iusd_market = calculateMarketValues(0, 0, 0);
  const copi_market = calculateMarketValues(0, 0, 0);
  const indy_market = calculateMarketValues(0, 0, 0);
  const newm_market = calculateMarketValues(0, 0, 0);
  const society_market = calculateMarketValues(0, 0, 0);
  const shen_market = calculateMarketValues(0, 0, 0);
  const milk_market = calculateMarketValues(0, 0, 0);
  const sundae_market = calculateMarketValues(0, 0, 0);
  const hosky_market = calculateMarketValues(0, 0, 0);
  const lq_market = calculateMarketValues(0, 0, 0);
  const cneta_market = calculateMarketValues(0, 0, 0);
  const bank_market = calculateMarketValues(0, 0, 0);
  const gens_market = calculateMarketValues(0, 0, 0);
  const vyfi_market = calculateMarketValues(0, 0, 0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const f_volumes = {
          f_turtle_price: Number(0).toFixed(0),
        ...
        if (complete_markets.froggie_market.price > 0)
        {
          setHaveData(true);
        ...
        <div>
          ...
          >
            <Grid 
              ...
              lg={12}
            >
              <StatusBanner sx={{ height: '100%', marginRight: '10px' }} haveData={haveData} error={haveError} coinName={'All'} />
            </Grid>
          </Grid>
        </div>
